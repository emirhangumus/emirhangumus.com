...
export default function Header() {
    ...
    return (
        <>
                ...
                <Container>
                    <div className="flex justify-between py-4 items-center">
                            ...
                            <div className="flex gap-4 items-center">
                                ...
                                <div>
                                    <div className="flex flex-col">
                                        ...
                                        <p className="lg:text-2xl text-sm text-zinc-500 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:bg-clip-text group-hover:from-pink-700 group-hover:to-blue-700">Full-Stack Developer</p>
                                    </div>
                                </div>
                            </div>
                        </Link>
                        ...
                        <div className="lg:hidden block">
                            <button className="flex items-center justify-center" aria-label="Menü" onClick={() => setMenuOpen(!menuOpen)}>
            ...
            {menuOpen && (
                <div className={`lg:hidden fixed top-0 left-0 w-full h-full bg-cinder-900 bg-opacity-95 z-20`}>
                    <FadeIn className="h-full" delay={0}>
                        ...
                        <div className="h-full grid place-items-center">
                            <ul className="flex flex-col gap-4">
                                {items.map(item => (
                                    <li key={item.id}>
                                        <Link href={item.href} className={`text-cinder-200 rounded-full block text-lg`}>
                                            {item.title}
                                        </Link>
                                    </li>
                                ))}
